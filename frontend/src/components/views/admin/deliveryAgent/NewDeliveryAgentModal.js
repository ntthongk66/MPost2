...
const NewDeliveryAgentModal = (props) => {
	...
	const handleNewDepartmentFormSubmit = async (formData) => {
		const data = {
			// receiverDetails: {
		...
		try {
			const url = `${apiHost}/api/admin/addDeliveryAgent`
			...
			const departmentEntryResponse = await response.json()
			...
			if (response.status === 204 || response.status === 201) {
				...
				toast.error(departmentEntryResponse.message, {
					position: 'top-right',
					autoClose: 5000,
					hideProgressBar: false,
					closeOnClick: true,
	...
	const handleDepartmentEntryFormSubmit = async (formData) => {
		const data = {
			staffDetails: {
				_id: formData._id,
			},
	...
	const branchNewDepartmentSchema = yup.object().shape({
                  ...
                  <Box display='flex' justifyContent='end' mt='20px'>
					...
					<TabPanel value={value} index={0}>
						...
						<Formik
							...
							onSubmit={handleNewDepartmentFormSubmit}
							validationSchema={branchNewDepartmentSchema}
						>
							{({
								values,
								errors,
									...
									>
										...
										>
											...
											<TextField
												...
												helperText={touched.senderPinCode && errors.pinCode}
												sx={{ gridColumn: 'span 1' }} />
...
export default NewDeliveryAgentModal