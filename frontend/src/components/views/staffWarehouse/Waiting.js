...
import CourierDetailModal from './courier/CourierDetailModal'
...
const CouriersWaiting = () => {
	...
	const [courierRowsList, setCourierRowsList] = useState(getRowsList(state.courier.couriers.Waiting))

	useEffect(() => {
		setCourierRowsList(getRowsList(state.courier.couriers.Waiting))
	}, [state])
								...
								.map((row, index) => {
									return (
										<TableRow hover key={row.id}>
											...
											{/* <TableCell>
												{row.status[`${state.auth.staffTransaction.transactionPointId}`]}
											</TableCell> fix id here */}
											<TableCell>
...
export default CouriersWaiting
