...
const StaffDetailModal = (props) => {
	...
	const warehouseId = auth.deliveryAgent._id
	...
	const handleStaffUpdateFormSubmit = async (formData) => {
		...
		try {
			const url = `${apiHost}/api/deliveryAgents/updateStaffInfo`
	...
	const handleDeleteStaffWarehouse = async () => {
		const data = {
			staffWarehouseDetails: { _id: props.data.id },
		}
		try {
			const url = `${apiHost}/api/deliveryAgents/deleteStaffWarehouse`
			const response = await fetch(url, {
				method: 'delete',
				body: JSON.stringify(data),
				headers: {
                            ...
                            Unsuccessful
											...
											<Box display='flex' justifyContent='end' mt='20px'>
												<Button
													type='text'
													variant='contained'
													onClick={handleDeleteStaffWarehouse}
													sx={{
														color: 'white',
