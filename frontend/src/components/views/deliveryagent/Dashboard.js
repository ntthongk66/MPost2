...
const Dashboard = () => {
	const couriersAccepted = useSelector((state) => state.courier.couriers.Accepted)
	const couriersWaiting  = useSelector((state) => state.courier.couriers.Waiting)
	
	const warehouseId = useSelector((state) => state.auth.deliveryAgent._id)
					...
					<Box mx={5} my={2} display={'flex'} alignItems='center'>
						...
						<Box>
							<Typography variant='h4' fontWeight={'bold'}>
								{couriersAccepted.length + couriersWaiting.length}
							</Typography>
			...
			<Box height={250} mt={2}>
				...
				<PieChart
					...
					data={[
						{
							title: 'Accepted',
							value: ((couriersAccepted.length / (couriersAccepted.length + couriersWaiting.length)) * 100).toFixed(1),
							color: '#E38627',
						},
						{
							title: 'Waiting',
							value: ((couriersWaiting.length / (couriersAccepted.length + couriersWaiting.length)) * 100).toFixed(1),
							color: '#C13C37',
						},
						// {
						// 	title: 'Unsuccessful',
