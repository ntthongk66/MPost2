...
export const loginStaffWarehouse = (credential) => {
  return (dispatch) => {
    const url = `${apiHost}/api/staffWarehouses/loginStaffWarehouse`
    fetch(url, {
      method: 'post',
      body: JSON.stringify({
        email: credential.email,
      ...
      .then((response) => {
        if (response.status === 404) {
          dispatch({
            type: 'STAFF_WAREHOUSE_NOT_FOUND',
          })
          toast.error('Warehouse Staff not found with given credentials', {
            position: 'top-right',
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
          ...
          response.json().then((res) => {
            dispatch({
              type: 'STAFF_WAREHOUSE_LOGIN_SUCCESS',
              payload: res.data,//
            })
          })
        }
